...
async function renderBudgets() {
  ...
  let html = `
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <!-- Большая карточка Общий бюджет -->
      <div class="lg:col-span-2 bg-white dark:bg-zinc-900 rounded-3xl p-8 border ${isOver ? 'border-red-500' : 'border-zinc-200 dark:border-zinc-800'} flex flex-col">
        ...
        <!-- БЛОК РАСХОДОВ ЗА МЕСЯЦ (большой и прокручиваемый) -->
  ...
  if (monthExpenses.length === 0) {
    ...
    monthExpenses.forEach(t => {
      html += `
        <div class="flex justify-between items-center bg-white dark:bg-zinc-900 rounded-3xl px-6 py-4 shadow-sm">
          <div>
